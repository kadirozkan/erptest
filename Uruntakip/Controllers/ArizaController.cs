...
namespace Uruntakip.Controllers
{
    public class ArizaController : Controller
    {
        ...
        public ActionResult _arizagecmisi(int makinaid)
        {
            ...
            List<tblarizalar> arizalistesi = db.tblarizalars.Where(x => x.makina_id == makinaid).ToList();
            foreach (var item in arizalistesi)
            {
                var liste = (from a in db.tblarizalars
                             join k in db.tblarizakategorileris on a.kategori_id equals k.ariza_kategori_ID
                             join i in db.tblarizaislemleris on a.arizaid equals i.ariza_id
        ...
        public ActionResult _arizaguncelle(int arizano, int kategori, string tanim, DateTime tarih)
        {
            ...
            try
            {
                tblarizalar t = db.tblarizalars.FirstOrDefault(x => x.arizaid == arizano);
        ...
        {
            tblarizalar aranan = db.tblarizalars.FirstOrDefault(x => x.arizaid == arizano);
            cls_arizalistesi c = new cls_arizalistesi();
        ...
        {
            tblmakina mc = db.tblmakinas.FirstOrDefault(x => x.makinaid == makinaid);
            List<tblarizakategorileri> liste = db.tblarizakategorileris.Where(x => x.makinatipi == mc.makinatip_id).ToList();
        ...
        public ActionResult yeniarizakaydi(int makinaid, int kategori, string tanım, DateTime tarih)
        {
            ...
            try
            {
                ...
                db.tblarizaislemleris.Add(islem);
                db.SaveChanges();
                sonuc = 1;
            }
            catch (Exception)
        ...
        {
            List<cls_arizalistesi> liste = (from i in db.tblarizaislemleris
                       join a in db.tblarizalars on i.ariza_id equals a.arizaid
                       join k in db.tblarizakategorileris on a.kategori_id equals k.ariza_kategori_ID
                       join s in db.tblarızasonuctipleri on i.islemtipi equals s.arizasonuc_id
                       join m in db.tblmakinas on a.makina_id equals m.makinaid
                       where i.ariza_id == id
        ...
        {
            List<cls_arizalistesi> liste = (from a in db.tblarizalars join k in db.tblarizakategorileris on a.kategori_id equals k.ariza_kategori_ID where a.makina_id == id && (a.durum != 4 && a.durum != 5) select (new cls_arizalistesi { _arizano = a.arizaid, _tanım = a.tanım })).ToList();
            return Json(liste, JsonRequestBehavior.AllowGet);
